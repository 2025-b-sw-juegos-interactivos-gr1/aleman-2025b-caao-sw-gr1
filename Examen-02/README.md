...
| **S** | Frenar / Reversa |
| **A** | Girar a la izquierda (maniobrabilidad depende de velocidad) |
| **D** | Girar a la derecha (maniobrabilidad depende de velocidad) |
| **ESPACIO** | Recoger / Entregar pedido |

## ✨ Características del Juego

- **Arquitectura Modular**: GameManager (Singleton), DeliveryManager (Observer), UIManager, CameraManager, PhysicsManager, ParticleManager, AudioManager
- **Cámara Dinámica**: FollowCamera que sigue desde atrás con FOV variable según velocidad
- **Física Avanzada**: Inercia refinada y maniobrabilidad que disminuye con velocidad (riesgo vs recompensa)
- **Efectos Visuales**: Partículas verdes de celebración al completar entregas
- **Audio Dinámico**: Pitch del motor que varía con la velocidad
...
- Centraliza todas las referencias del juego
- Inicializa y coordina todos los managers
- Gestiona el estado global del juego

### Patrón Observer: DeliveryManager
- Subject que notifica eventos de entrega
- Gestiona la lógica de recoger/entregar pedidos
- Separa la lógica de negocio de la UI

### Managers Especializados
- **UIManager**: Observer que actualiza la interfaz sin acoplamiento
- **CameraManager**: Maneja la FollowCamera con FOV dinámico
- **PhysicsManager**: Física refinada con maniobrabilidad variable
- **ParticleManager**: Efectos de partículas para feedback visual
...
```
/
├── index.html          # Archivo principal con escena y HTML
├── GameManager.js      # Singleton principal
├── DeliveryManager.js  # Observer Subject para entregas
├── UIManager.js        # Observer para UI
├── CameraManager.js    # Cámara dinámica
├── PhysicsManager.js   # Física del vehículo
├── ParticleManager.js  # Efectos de partículas
├── AudioManager.js     # Audio del motor
├── models/             # Modelos 3D (moto.glb)
└── textures/           # Texturas del juego
```

## 🔧 Mejoras Implementadas

### 1. Arquitectura Formalizada
- **GameManager como Singleton**: Eliminación de variables globales
- **DeliveryManager como Observer**: UIManager se suscribe a eventos sin acoplamiento directo
- **Separación de responsabilidades**: Cada manager maneja un aspecto específico

### 2. Cámara Dinámica
- **FollowCamera**: Sigue a la moto desde atrás en lugar de vista isométrica
- **FOV Dinámico**: Aumenta linealmente con velocidadActual para sensación de urgencia
- **Configuración optimizada**: Radio, heightOffset y suavidad ajustados

### 3. Físicas y Manejo Refinadas
- **Inercia mejorada**: Más realista y predecible
- **Maniobrabilidad variable**: Disminuye a altas velocidades (0.5 factor)
- **Dinámica Riesgo vs Recompensa**: Velocidad alta = giros más amplios y difíciles

### 4. Feedback Visual (Juice)
- **Partículas verdes**: Estallido de celebración al completar Dropoff
- **Sistema de partículas**: Configurado con colores, tamaños y direcciones apropiadas
- **Duración limitada**: 1 segundo de emisión

### 5. Audio Dinámico
- **Pitch variable**: Motor suena más agudo a mayor velocidad
- **Audio procedural**: Oscilador sinusoidal para simular sonido del motor
- **Rango de frecuencia**: 200Hz base hasta 400Hz máximo
...
3. **Controles**: Usa W/A/S/D y ESPACIO según las instrucciones en pantalla

## 🎯 Vertical Slice Completado

Esta refactorización completa el **Vertical Slice técnico** con:
- ✅ Arquitectura modular y desacoplada
- ✅ Cámara dinámica con feedback visual
- ✅ Físicas avanzadas con mecánicas de riesgo/recompensa
- ✅ Sistema de partículas y audio integrado
- ✅ Código limpio sin variables globales
